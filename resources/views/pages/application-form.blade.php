...
@if(session('payment_error') || session('payment_info'))
    ...
    >
        @if(session('payment_error'))
            <div
                class="pointer-events-auto flex items-start gap-3 w-full max-w-xl rounded-xl shadow-2xl px-5 py-4 text-sm bg-red-600 text-white ring-1 ring-red-700">
                <svg class="mt-0.5 h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                     viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
                          d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008z"/>
                </svg>
                <span class="flex-1 font-semibold">{{ session('payment_error') }}</span>
                <button @click="show = false" class="opacity-80 hover:opacity-100 transition text-lg leading-none">
                    &times;
                </button>
            </div>
        @else
            <div
                class="pointer-events-auto flex items-start gap-3 w-full max-w-xl rounded-xl shadow-2xl px-5 py-4 text-sm bg-amber-500 text-white ring-1 ring-amber-600">
                <svg class="mt-0.5 h-5 w-5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                     viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
...
>
    <div class="w-full max-w-3xl max-h-[90vh] flex flex-col rounded-xl bg-white shadow-2xl border border-gray-200">
        <div class="px-6 py-5 border-b border-gray-200 flex-shrink-0">
            ...
            <p class="text-sm text-gray-600 mt-1">Please review these instructions carefully. Application fields will be
                enabled after you confirm.</p>
        </div>
        ...
        <div class="px-6 py-5 space-y-5 text-sm text-gray-700 overflow-y-auto flex-1">
            ...
            <div>
                ...
                <ul class="list-disc list-inside space-y-1">
                    <li>Photo: {{ data_get($photoRules, 'width', 300) }}x{{ data_get($photoRules, 'height', 300) }} px,
                        max {{ data_get($photoRules, 'max_kb', 1024) }} KB.
                    </li>
                    <li>Signature: {{ data_get($signatureRules, 'width', 300) }}
                        x{{ data_get($signatureRules, 'height', 80) }} px,
                        max {{ data_get($signatureRules, 'max_kb', 512) }} KB.
                    </li>
                    <li>SSC, HSC, and other qualification certificate PDFs:
                        max {{ data_get($certificateRules, 'max_kb', 5120) }} KB each.
                    </li>
                </ul>
            </div>
            ...
            <div>
                ...
                <ol class="list-decimal list-inside space-y-1">
                    ...
                    <li><strong>Education</strong>: SSC, HSC, Graduation, optional Masters, and optional MPhil/PhD
                        details.
                    </li>
            ...
            <div class="rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
                ...
                <ul class="list-disc list-inside mt-2 space-y-1">
                    ...
                    <li>If submission fails for validation/duplicate checks, your uploaded photo, signature, and
                        certificate PDFs are cleared and must be uploaded again.
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-end gap-2 flex-shrink-0">
            <a href="{{ route('home') }}"
               class="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Back</a>
            <button
                type="button"
...
<div :class="showIntroModal ? 'pointer-events-none select-none opacity-65 blur-[2px]' : ''">
    <header class="bg-white border-b border-gray-200">
        <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <a href="{{ route('home') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">Back to
                Homepage</a>
        </div>
    </header>
    ...
    <main class="max-w-6xl mx-auto px-4 py-8">
        <div
            class="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 mb-6 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <span class="text-sm font-semibold text-blue-900">Application period:</span>
            <span
                class="text-sm text-blue-800">{{ $applicationStartAt ?? 'Now' }} to {{ $applicationEndAt ?? 'Until exam closes' }}</span>
        ...
        @if ($errors->any())
            <div class="mb-6 rounded-lg border border-red-200 bg-red-50 p-4">
                ...
                @if ($hasUploadErrors)
                    <p class="mt-2 text-sm font-medium text-red-700">Uploads are not retained after failed attempts.
                        Please re-upload required photo/signature/certificate files.</p>
                @endif
        ...
        >
            ...
            <section x-show="step === 1" x-cloak
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="applicant_name" class="block text-sm font-medium text-gray-700">Applicant's Name
                            *</label>
                        <input id="applicant_name" name="applicant_name" type="text" value="{{ old('applicant_name') }}"
                    ...
                    <div>
                        <label for="national_id_number" class="block text-sm font-medium text-gray-700">National ID /
                            Birth Reg. / Passport *</label>
                        <input id="national_id_number" name="national_id_number" type="text"
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" class="mt-1 block w-full rounded-md border-gray-300" required>
                            ...
                            @foreach (($formOptions['genders'] ?? []) as $gender)
                                <option
                                    value="{{ $gender }}" @selected(old('gender') === $gender)>{{ $gender }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div>
                        <label for="age_as_of_reference" class="block text-sm font-medium text-gray-700">Age (as of
                            today) *</label>
                        <input id="age_as_of_reference" type="text" x-model="ageDisplay"
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <span
                                class="inline-flex items-center rounded-l-md border border-r-0 border-gray-300 bg-gray-100 px-3 text-sm text-gray-700">+880</span>
                            <input
                                id="mobile_number"
                        ...
                        <p class="mt-1 text-xs text-gray-500">Enter 10 digits only. Country code +880 is added
                            automatically.</p>
                    </div>
                    ...
                    <div id="uniquenesAlert" x-show="showUniquenessAlert" x-cloak
                         class="rounded-lg border-l-4 border-red-400 bg-red-50 p-4">
                        <div class="flex items-start gap-3">
                            <svg class="h-5 w-5 text-red-600 mt-0.5 flex-shrink-0" fill="currentColor"
                                 viewBox="0 0 20 20">
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            <label for="applicant_photo" class="block text-sm font-medium text-gray-700">Applicant Photo
                                * (300x300 pixels, max 1MB)</label>
                            <div x-show="photoPreviewUrl" x-cloak class="rounded border border-gray-200 bg-gray-50 p-1">
                                <img :src="photoPreviewUrl" alt="Applicant photo preview"
                                     class="h-10 w-10 rounded object-cover border border-gray-200">
                            </div>
                        </div>
                        @if($existingPhoto)
                            <p class="mt-1 text-xs text-emerald-700 font-medium">✓ Previously uploaded photo retained.
                                Upload a new file below to replace it.</p>
                        @endif
                        <input id="applicant_photo" name="applicant_photo" type="file" accept="image/*"
                               x-on:change="handleImagePreview($event, 'photo')" class="mt-1 block w-full text-sm"
                               required>
                        <p class="text-xs text-red-600 mt-1" x-text="fileErrors.photo"></p>
                    </div>

                    <div>
                        <div class="flex items-center justify-between">
                            <label for="signature_input" class="block text-sm font-medium text-gray-700">Signature *
                                (300x80 pixels, max 1MB)</label>
                            <div x-show="signaturePreviewUrl" x-cloak
                                 class="rounded border border-gray-200 bg-gray-50 p-1">
                                <img :src="signaturePreviewUrl" alt="Applicant signature preview"
                                     class="h-8 w-24 rounded object-contain border border-gray-200 bg-white">
                            </div>
                        </div>
                        @if($existingSignature)
                            <p class="mt-1 text-xs text-emerald-700 font-medium">✓ Previously uploaded signature
                                retained. Upload a new file below to replace it.</p>
                        @endif
                        <input id="signature_input" name="signature" type="file" accept="image/*"
                               x-on:change="handleImagePreview($event, 'signature')" class="mt-1 block w-full text-sm"
                               required>
                        <p class="text-xs text-red-600 mt-1" x-text="fileErrors.signature"></p>
                    </div>
                </div>
            </section>

            <section x-show="step === 2" x-cloak
                     class="bg-white border border-gray-200 rounded-lg p-4 sm:p-6 space-y-5">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="grid grid-cols-1 gap-3 mt-2">
                            ...
                            <div class="relative" :class="presentDistrictOpen ? 'pb-52' : ''">
                                ...
                                <div x-show="presentDistrictOpen" x-cloak
                                    ...
                                    <div x-show="filteredDistricts(presentDistrictText).length === 0"
                                         class="px-3 py-2 text-sm text-gray-400 italic">No districts found
                                    </div>
                                </div>
                            ...
                            <div class="relative" :class="presentUpazilaOpen ? 'pb-52' : ''">
                                ...
                                >
                                <div x-show="presentUpazilaOpen" x-cloak
                                     class="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                    <template x-for="upazila in filteredUpazilas(presentDistrictId, presentUpazilaText)"
                                              :key="upazila.id">
                                        <button
                                            type="button"
                                            class="block w-full text-left px-3 py-2 text-sm hover:bg-indigo-50"
                                    ...
                                    <div x-show="filteredUpazilas(presentDistrictId, presentUpazilaText).length === 0"
                                         class="px-3 py-2 text-sm text-gray-400 italic">No upazila/thana found
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <input name="present_address[post_office]" type="text"
                                       value="{{ old('present_address.post_office') }}"
                                       x-on:input="if (sameAsPresentAddress) syncPermanentFromPresent()"
                                       placeholder="Post Office" class="rounded-md border-gray-300 w-full" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Post Code *</label>
                                <input name="present_address[post_code]" type="text"
                                       value="{{ old('present_address.post_code') }}"
                                       x-on:input="if (sameAsPresentAddress) syncPermanentFromPresent()"
                                       placeholder="Post Code" class="rounded-md border-gray-300 w-full" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Village / Road / House /
                                    Flat *</label>
                                <input name="present_address[address_line]" type="text"
                                       value="{{ old('present_address.address_line') }}"
                                       x-on:input="if (sameAsPresentAddress) syncPermanentFromPresent()"
                                       placeholder="Village/Road/House/Flat" class="rounded-md border-gray-300 w-full"
                                       required>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="rounded-lg border border-gray-200 p-4">
                        ...
                        <label
                            class="mt-2 inline-flex items-center gap-2 rounded-md border border-indigo-100 bg-indigo-50 px-3 py-2 text-sm text-indigo-900">
                            <input type="checkbox" name="same_as_present_address" value="1"
                                   x-model="sameAsPresentAddress" x-on:change="toggleSameAsPresentAddress()"
                        ...
                        <div class="grid grid-cols-1 gap-3 mt-2">
                            ...
                            <div class="relative" :class="permanentDistrictOpen ? 'pb-52' : ''">
                                ...
                                >
                                <div x-show="permanentDistrictOpen" x-cloak
                                     class="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                    <template x-for="district in filteredDistricts(permanentDistrictText)"
                                              :key="district.id">
                                        <button
                                            type="button"
                                            class="block w-full text-left px-3 py-2 text-sm hover:bg-indigo-50"
                                    ...
                                    <div x-show="filteredDistricts(permanentDistrictText).length === 0"
                                         class="px-3 py-2 text-sm text-gray-400 italic">No districts found
                                    </div>
                                </div>
                            ...
                            <div class="relative" :class="permanentUpazilaOpen ? 'pb-52' : ''">
                                ...
                                <input
                                    ...
                                    :class="sameAsPresentAddress ? 'bg-gray-100 cursor-not-allowed' : ''"
                                >
                                <div x-show="permanentUpazilaOpen" x-cloak
                                     class="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                    <template
                                        x-for="upazila in filteredUpazilas(permanentDistrictId, permanentUpazilaText)"
                                        :key="upazila.id">
                                        <button
                                            type="button"
                                            class="block w-full text-left px-3 py-2 text-sm hover:bg-indigo-50"
                                    ...
                                    <div
                                        x-show="filteredUpazilas(permanentDistrictId, permanentUpazilaText).length === 0"
                                        class="px-3 py-2 text-sm text-gray-400 italic">No upazila/thana found
                                    </div>
                                </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Post Office *</label>
                                <input name="permanent_address[post_office]" type="text"
                                       value="{{ old('permanent_address.post_office') }}"
                                       :readonly="sameAsPresentAddress" placeholder="Post Office"
                                       class="rounded-md border-gray-300 w-full"
                                       :class="sameAsPresentAddress ? 'bg-gray-100' : ''" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Post Code *</label>
                                <input name="permanent_address[post_code]" type="text"
                                       value="{{ old('permanent_address.post_code') }}" :readonly="sameAsPresentAddress"
                                       placeholder="Post Code" class="rounded-md border-gray-300 w-full"
                                       :class="sameAsPresentAddress ? 'bg-gray-100' : ''" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Village / Road / House /
                                    Flat *</label>
                                <input name="permanent_address[address_line]" type="text"
                                       value="{{ old('permanent_address.address_line') }}"
                                       :readonly="sameAsPresentAddress" placeholder="Village/Road/House/Flat"
                                       class="rounded-md border-gray-300 w-full"
                                       :class="sameAsPresentAddress ? 'bg-gray-100' : ''" required>
                            </div>
                        </div>
            ...
            <section x-show="step === 3" x-cloak
                     class="bg-white border border-gray-200 rounded-lg p-4 sm:p-6 space-y-6">
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Examination *</label><select
                                name="education[ssc][examination]" class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Examination
                                </option>@foreach ($formOptions['ssc_examinations'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.ssc.examination') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Education Board
                                *</label><select name="education[ssc][education_board]"
                                                 class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Education Board
                                </option>@foreach ($formOptions['education_boards'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.ssc.education_board') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Style *</label>
                            <select name="education[ssc][result_type]" x-model="educationResultTypes.ssc"
                                    class="rounded-md border-gray-300 w-full" required>
                        ...
                        <div x-show="educationResultTypes.ssc === 'numeric'" x-cloak>
                            ...
                            <input name="education[ssc][result_scale]" type="number" step="0.001" min="0"
                                   value="{{ old('education.ssc.result_scale') }}" placeholder="e.g. 5.00"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.ssc === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.ssc === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">GPA *</label>
                            <input name="education[ssc][result]" type="number" step="0.001" min="0"
                                   value="{{ old('education.ssc.result') }}" placeholder="e.g. 4.67"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.ssc === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.ssc === 'division'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Division *</label>
                            <select name="education[ssc][division]" class="rounded-md border-gray-300 w-full"
                                    :required="educationResultTypes.ssc === 'division'">
                                <option value="">Select Division</option>
                                @foreach ($educationDivisions as $division)
                                    <option
                                        value="{{ $division }}" @selected(old('education.ssc.division') === $division)>{{ $division }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Group *</label><select
                                name="education[ssc][group]" class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Group</option>@foreach ($formOptions['groups'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.ssc.group') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Passing Year *</label><input
                                name="education[ssc][passing_year]" type="number"
                                value="{{ old('education.ssc.passing_year') }}" placeholder="Passing Year"
                                class="rounded-md border-gray-300 w-full" required></div>
                    </div>
                    <div class="grid grid-cols-1 gap-3 mt-4">
                        <div>
                            <label for="ssc_certificate"
                                   class="flex items-center gap-2 text-sm font-medium text-gray-700">SSC Certificate PDF
                                *<a x-show="pdfPreviewUrls.ssc_certificate" x-cloak
                                    :href="pdfPreviewUrls.ssc_certificate" target="_blank" rel="noopener"
                                    class="inline-flex items-center rounded-md border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">View
                                    Upload</a></label>
                            <input id="ssc_certificate" name="education_documents[ssc][certificate]" type="file"
                                   accept="application/pdf" x-on:change="handlePdfPreview($event, 'ssc_certificate')"
                                   class="mt-1 block w-full text-sm" required>
                            <p class="text-xs text-red-600 mt-1" x-text="fileErrors['ssc_certificate']"></p>
                        </div>
                    </div>
                </fieldset>
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Examination *</label><select
                                name="education[hsc][examination]" class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Examination
                                </option>@foreach ($formOptions['hsc_examinations'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.hsc.examination') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Education Board
                                *</label><select name="education[hsc][education_board]"
                                                 class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Education Board
                                </option>@foreach ($formOptions['education_boards'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.hsc.education_board') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Style *</label>
                            <select name="education[hsc][result_type]" x-model="educationResultTypes.hsc"
                                    class="rounded-md border-gray-300 w-full" required>
                                @foreach ($educationResultTypes as $resultTypeKey => $resultTypeLabel)
                                    <option value="{{ $resultTypeKey }}">{{ $resultTypeLabel }}</option>
                                @endforeach
                        ...
                        <div x-show="educationResultTypes.hsc === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Scale *</label>
                            <input name="education[hsc][result_scale]" type="number" step="0.001" min="0"
                                   value="{{ old('education.hsc.result_scale') }}" placeholder="e.g. 5.00"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.hsc === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.hsc === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">GPA *</label>
                            <input name="education[hsc][result]" type="number" step="0.001" min="0"
                                   value="{{ old('education.hsc.result') }}" placeholder="e.g. 4.50"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.hsc === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.hsc === 'division'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Division *</label>
                            <select name="education[hsc][division]" class="rounded-md border-gray-300 w-full"
                                    :required="educationResultTypes.hsc === 'division'">
                                <option value="">Select Division</option>
                                @foreach ($educationDivisions as $division)
                                    <option
                                        value="{{ $division }}" @selected(old('education.hsc.division') === $division)>{{ $division }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Group *</label><select
                                name="education[hsc][group]" class="rounded-md border-gray-300 w-full" required>
                                <option value="">Select Group</option>@foreach ($formOptions['groups'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.hsc.group') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Passing Year *</label><input
                                name="education[hsc][passing_year]" type="number"
                                value="{{ old('education.hsc.passing_year') }}" placeholder="Passing Year"
                                class="rounded-md border-gray-300 w-full" required></div>
                    </div>
                    <div class="grid grid-cols-1 gap-3 mt-4">
                        <div>
                            <label for="hsc_certificate"
                                   class="flex items-center gap-2 text-sm font-medium text-gray-700">HSC Certificate PDF
                                *<a x-show="pdfPreviewUrls.hsc_certificate" x-cloak
                                    :href="pdfPreviewUrls.hsc_certificate" target="_blank" rel="noopener"
                                    class="inline-flex items-center rounded-md border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">View
                                    Upload</a></label>
                            <input id="hsc_certificate" name="education_documents[hsc][certificate]" type="file"
                                   accept="application/pdf" x-on:change="handlePdfPreview($event, 'hsc_certificate')"
                                   class="mt-1 block w-full text-sm" required>
                            <p class="text-xs text-red-600 mt-1" x-text="fileErrors['hsc_certificate']"></p>
                        </div>
                    </div>
                </fieldset>
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Examination *</label><select
                                name="education[graduation][examination]" class="rounded-md border-gray-300 w-full"
                                required>
                                <option value="">Select Examination
                                </option>@foreach ($formOptions['graduation_examinations'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('education.graduation.examination') === $option)>{{ $option }}</option>
                                @endforeach</select></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Subject *</label><input
                                name="education[graduation][subject]" type="text"
                                value="{{ old('education.graduation.subject') }}" placeholder="Subject"
                                class="rounded-md border-gray-300 w-full" required></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">University / Institute
                                *</label><input name="education[graduation][institution]" type="text"
                                                value="{{ old('education.graduation.institution') }}"
                                                placeholder="University / Institute"
                                                class="rounded-md border-gray-300 w-full" required></div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Style *</label>
                            <select name="education[graduation][result_type]" x-model="educationResultTypes.graduation"
                                    class="rounded-md border-gray-300 w-full" required>
                                @foreach ($educationResultTypes as $resultTypeKey => $resultTypeLabel)
                                    <option value="{{ $resultTypeKey }}">{{ $resultTypeLabel }}</option>
                                @endforeach
                        ...
                        <div x-show="educationResultTypes.graduation === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Scale *</label>
                            <input name="education[graduation][result_scale]" type="number" step="0.001" min="0"
                                   value="{{ old('education.graduation.result_scale') }}" placeholder="e.g. 4.00"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.graduation === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.graduation === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">CGPA *</label>
                            <input name="education[graduation][result]" type="number" step="0.001" min="0"
                                   value="{{ old('education.graduation.result') }}" placeholder="e.g. 3.75"
                                   class="rounded-md border-gray-300 w-full"
                                   :required="educationResultTypes.graduation === 'numeric'">
                        </div>
                        <div x-show="educationResultTypes.graduation === 'division'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Division *</label>
                            <select name="education[graduation][division]" class="rounded-md border-gray-300 w-full"
                                    :required="educationResultTypes.graduation === 'division'">
                                <option value="">Select Division</option>
                                @foreach ($educationDivisions as $division)
                                    <option
                                        value="{{ $division }}" @selected(old('education.graduation.division') === $division)>{{ $division }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Passing Year *</label><input
                                name="education[graduation][passing_year]" type="number"
                                value="{{ old('education.graduation.passing_year') }}" placeholder="Passing Year"
                                class="rounded-md border-gray-300 w-full" required></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Course Duration (Years)
                                *</label><input name="education[graduation][course_duration_years]" type="number"
                                                step="0.1"
                                                value="{{ old('education.graduation.course_duration_years') }}"
                    ...
                    <div class="grid grid-cols-1 gap-3 mt-4">
                        <div>
                            <label for="graduation_certificate"
                                   class="flex items-center gap-2 text-sm font-medium text-gray-700">Graduation
                                Certificate PDF *<a x-show="pdfPreviewUrls.graduation_certificate" x-cloak
                                                    :href="pdfPreviewUrls.graduation_certificate" target="_blank"
                                                    rel="noopener"
                                                    class="inline-flex items-center rounded-md border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">View
                                    Upload</a></label>
                            <input id="graduation_certificate" name="education_documents[graduation][certificate]"
                                   type="file" accept="application/pdf"
                                   x-on:change="handlePdfPreview($event, 'graduation_certificate')"
                                   class="mt-1 block w-full text-sm" required>
                            <p class="text-xs text-red-600 mt-1" x-text="fileErrors['graduation_certificate']"></p>
                        </div>
                    </div>
                </fieldset>

                <fieldset class="rounded-lg border border-gray-200 p-4">
                    <legend class="px-2 text-sm font-semibold text-gray-700">Masters / Equivalent (If Applicable)
                    </legend>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Examination</label><input
                                name="education[masters][examination]" type="text"
                                value="{{ old('education.masters.examination') }}" placeholder="Examination"
                                class="rounded-md border-gray-300 w-full"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Subject</label><input
                                name="education[masters][subject]" type="text"
                                value="{{ old('education.masters.subject') }}" placeholder="Subject"
                                class="rounded-md border-gray-300 w-full"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">University /
                                Institute</label><input name="education[masters][institution]" type="text"
                                                        value="{{ old('education.masters.institution') }}"
                                                        placeholder="University / Institute"
                                                        class="rounded-md border-gray-300 w-full"></div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Style</label>
                            <select name="education[masters][result_type]" x-model="educationResultTypes.masters"
                                    class="rounded-md border-gray-300 w-full">
                        ...
                        <div x-show="educationResultTypes.masters === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result Scale</label>
                            <input name="education[masters][result_scale]" type="number" step="0.001" min="0"
                                   value="{{ old('education.masters.result_scale') }}" placeholder="e.g. 4.00"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div x-show="educationResultTypes.masters === 'numeric'" x-cloak>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Result</label>
                            <input name="education[masters][result]" type="number" step="0.001" min="0"
                                   value="{{ old('education.masters.result') }}" placeholder="Result"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div x-show="educationResultTypes.masters === 'division'" x-cloak>
                            ...
                            <select name="education[masters][division]" class="rounded-md border-gray-300 w-full">
                                ...
                                @foreach ($educationDivisions as $division)
                                    <option
                                        value="{{ $division }}" @selected(old('education.masters.division') === $division)>{{ $division }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Passing Year</label><input
                                name="education[masters][passing_year]" type="number"
                                value="{{ old('education.masters.passing_year') }}" placeholder="Passing Year"
                                class="rounded-md border-gray-300 w-full"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Course Duration (Years)</label><input
                                name="education[masters][course_duration_years]" type="number" step="0.1"
                                value="{{ old('education.masters.course_duration_years') }}"
                                placeholder="Course Duration (Years)" class="rounded-md border-gray-300 w-full"></div>
                    </div>
                    <div class="grid grid-cols-1 gap-3 mt-4">
                        <div>
                            <label for="masters_certificate"
                                   class="flex items-center gap-2 text-sm font-medium text-gray-700">Masters Certificate
                                PDF (Optional)<a x-show="pdfPreviewUrls.masters_certificate" x-cloak
                                                 :href="pdfPreviewUrls.masters_certificate" target="_blank"
                                                 rel="noopener"
                                                 class="inline-flex items-center rounded-md border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">View
                                    Upload</a></label>
                            <input id="masters_certificate" name="education_documents[masters][certificate]" type="file"
                                   accept="application/pdf"
                                   x-on:change="handlePdfPreview($event, 'masters_certificate')"
                                   class="mt-1 block w-full text-sm">
                            <p class="text-xs text-red-600 mt-1" x-text="fileErrors['masters_certificate']"></p>
                        </div>
                    </div>
                </fieldset>
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Subject</label><input
                                name="education[mphil_phd][subject]" type="text"
                                value="{{ old('education.mphil_phd.subject') }}" placeholder="Subject"
                                class="rounded-md border-gray-300 w-full"></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">University /
                                Institute</label><input name="education[mphil_phd][institution]" type="text"
                                                        value="{{ old('education.mphil_phd.institution') }}"
                                                        placeholder="University / Institute"
                                                        class="rounded-md border-gray-300 w-full"></div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Degree Completion Status</label>
                            <select name="education[mphil_phd][degree_completion]"
                                    class="rounded-md border-gray-300 w-full">
                                <option value="">— Select Status —</option>
                                <option
                                    value="degree_awarded" @selected(old('education.mphil_phd.degree_completion') === 'degree_awarded')>
                                    Degree Awarded
                                </option>
                                <option
                                    value="ongoing" @selected(old('education.mphil_phd.degree_completion') === 'ongoing')>
                                    Ongoing
                                </option>
                            </select>
                        </div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">Completion Year</label><input
                                name="education[mphil_phd][completion_year]" type="number"
                                value="{{ old('education.mphil_phd.completion_year') }}" placeholder="Year (optional)"
                                class="rounded-md border-gray-300 w-full"></div>
                    </div>
                </fieldset>
            </section>
            ...
            <section x-show="step === 4" x-cloak
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4 space-y-3">
                    ...
                    <div>
                        ...
                        <input name="job_experience[total_years]" type="number" min="0" step="0.1"
                               value="{{ old('job_experience.total_years', 0) }}"
                               placeholder="Total Job Experience (Years)" class="rounded-md border-gray-300 w-full">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="job_experience[current][job_category]"
                                    class="rounded-md border-gray-300 w-full">
                                <option value="">Select Job Category</option>
                                @foreach ($formOptions['job_categories'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('job_experience.current.job_category') === $option)>{{ $option }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Organization Name</label>
                            <input name="job_experience[current][organization_name]" type="text"
                                   value="{{ old('job_experience.current.organization_name') }}"
                                   placeholder="Organization Name" class="rounded-md border-gray-300 w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Current Designation /
                                Position</label>
                            <input name="job_experience[current][designation]" type="text"
                                   value="{{ old('job_experience.current.designation') }}"
                                   placeholder="Current Designation / Position"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Starting Date</label>
                            <input name="job_experience[current][starting_date]" type="date"
                                   value="{{ old('job_experience.current.starting_date') }}"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                            <input name="job_experience[current][address]" type="text"
                                   value="{{ old('job_experience.current.address') }}" placeholder="Address"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Job Category</label>
                            <select name="job_experience[previous][job_category]"
                                    class="rounded-md border-gray-300 w-full">
                                <option value="">Select Job Category</option>
                                @foreach ($formOptions['job_categories'] as $option)
                                    <option
                                        value="{{ $option }}" @selected(old('job_experience.previous.job_category') === $option)>{{ $option }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Organization Name</label>
                            <input name="job_experience[previous][organization_name]" type="text"
                                   value="{{ old('job_experience.previous.organization_name') }}"
                                   placeholder="Organization Name" class="rounded-md border-gray-300 w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Designation / Post</label>
                            <input name="job_experience[previous][designation]" type="text"
                                   value="{{ old('job_experience.previous.designation') }}"
                                   placeholder="Designation / Post" class="rounded-md border-gray-300 w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Starting Date</label>
                            <input name="job_experience[previous][starting_date]" type="date"
                                   value="{{ old('job_experience.previous.starting_date') }}"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Ending Date</label>
                            <input name="job_experience[previous][ending_date]" type="date"
                                   value="{{ old('job_experience.previous.ending_date') }}"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                            <input name="job_experience[previous][address]" type="text"
                                   value="{{ old('job_experience.previous.address') }}" placeholder="Address"
                                   class="rounded-md border-gray-300 w-full">
                        </div>
                    </div>
                </fieldset>
            ...
            <section x-show="step === 5" x-cloak
                ...
                <fieldset class="rounded-lg border border-gray-200 p-4">
                    ...
                    <p class="text-sm text-gray-500 mt-1 mb-3">All 6 choices are required. Each program must be selected
                        exactly once.</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        <template x-for="(choice, idx) in courseChoiceFields" :key="choice.field">
                            <div>
                                ...
                                <p x-show="isDuplicateChoice(idx)" class="mt-1 text-xs text-red-600">Already selected in
                                    another choice.</p>
                            </div>
                        </template>
                    </div>
            ...
            <section x-show="step === 6" x-cloak
                ...
                <div class="rounded-lg border border-indigo-100 bg-indigo-50 p-4">
                    <p class="text-sm text-indigo-900">
                        You do not need to enter payment details now. After confirming this form, you will be redirected
                        to the payment gateway.
                    </p>
                </div>

                <label class="flex items-start gap-2 rounded-md border border-gray-200 bg-gray-50 p-3">
                    <input name="declaration" type="checkbox" value="1" class="mt-1 rounded border-gray-300"
                ...
                <label class="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3">
                    <input name="contact_info_confirmation" type="checkbox" value="1"
                           class="mt-1 rounded border-gray-300"
                           {{ old('contact_info_confirmation') ? 'checked' : '' }} required>
                    <span class="text-sm text-gray-700">
            ...
            <div x-show="stepErrors.length > 0" x-cloak class="rounded-lg border border-red-200 bg-red-50 p-4">
                <p class="text-sm font-semibold text-red-800 mb-1">Please complete the required fields before
                    continuing:</p>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="ml-auto flex items-center gap-3">
                    ...
                    >
                        <svg x-show="isCheckingUniqueness" x-cloak class="mr-2 h-4 w-4 animate-spin"
    ...
    function applicationStepper({
        ...
        return {
            ...
            init() {
                ...
                this.$watch('presentDistrictId', v => {
                    if (v) this._clearInvalid(document.getElementById('present_district_input'));
                });
                this.$watch('presentUpazilaId', v => {
                    if (v) this._clearInvalid(document.getElementById('present_upazila_input'));
                });
                this.$watch('permanentDistrictId', v => {
                    if (v) this._clearInvalid(document.getElementById('permanent_district_input'));
                });
                this.$watch('permanentUpazilaId', v => {
                    if (v) this._clearInvalid(document.getElementById('permanent_upazila_input'));
                });
            },
            ...
            async handleImagePreview(event, type) {
                const file = event?.target?.files?.[0] ?? null;

                if (!file) return;

                const check = await this.validateImage(file, type);

                if (!check.valid) {
                    this.fileErrors[type] = check.message;
                    event.target.value = '';
                    return;
                }

                delete this.fileErrors[type];
            ...
            handlePdfPreview(event, key) {
                const maxCertificateSizeMB = 1;
                const file = event?.target?.files?.[0] ?? null;

                if (!file) return;

                const sizeCheck = this.validateFileSize(file, maxCertificateSizeMB);

                if (!sizeCheck.valid) {
                    this.fileErrors[key] = sizeCheck.message;
                    event.target.value = '';
                    return;
                }
                ...
                this.pdfPreviewUrls[key] = URL.createObjectURL(file);
            },
            clearUploadedFiles() {
                const fileInputIds = [
                    'applicant_photo',
            ...
            async next() {
                if (Object.keys(this.fileErrors).length > 0) {
                    this.stepErrors = Object.values(this.fileErrors);
            ...
            async submitWithUniquenessCheck(event) {
                if (Object.keys(this.fileErrors).length > 0) {
                    this.step = 3; // or step where upload happens
                    this.stepErrors = Object.values(this.fileErrors);
                    return;
                }
                if (this.step !== this.totalSteps) {
            ...
            async validateImage(file, type) {
                const sizeCheck = this.validateFileSize(file, 1); // 1MB limit for ALL images
            ...
            validateFileSize(file, maxMB) {
                ...
                if (sizeMB > maxMB) {
                    return {
                        valid: false,
                        message: `File must be less than ${maxMB}MB (current: ${sizeMB.toFixed(2)}MB)`
                    };
                }
